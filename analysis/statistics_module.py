...
class Classifierbinomial:
    ...
    def binomial_probability(self):
        ...
        accuracy_probability = round(
            1 - binom.cdf(self.correct_class, self.total, self.null_probability), 3
        )
    ...
    def accuracy_confidence_interval(self, confidence=0.05):
        ...
        conf_interval = proportion_confint(
            count=self.correct_class, nobs=self.total, alpha=confidence, method="beta"
        )
...
class Statistics:
    def friedman_test(self, data):
        ...
        # If any value in any column is nan, remove it from all columns
        nan_indices = np.argwhere(np.isnan(data))
        data_columns = [np.delete(column, nan_indices) for column in data_columns]
    ...
    def wilcoxon_test(self, data_1, data_2):
        ...
        # If any value in data_1 or data_2 is nan, remove it from both data sets
        nan_indices = np.argwhere(np.isnan(data_1) | np.isnan(data_2))
        data_1 = np.delete(data_1, nan_indices)
        data_2 = np.delete(data_2, nan_indices)

        if data_1.shape != data_2.shape:
            raise Exception("Data sets must have the same shape")
...
if __name__ == "__main__":
    ...
    accuracy, lower_bound, upper_bound = classtest.accuracy_confidence_interval(
        alpha_confidence
    )
    print(
        f"Confidence interval for true accuracy percentage is: {lower_bound, upper_bound} with {100-(alpha_confidence*100)} % certainty "
    )
